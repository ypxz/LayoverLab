...
ERROR_BACKOFF = timedelta(hours=2)
...
def claim_next_job(session: Session) -> CrawlJob | None:
    stmt = (
        select(CrawlJob)
        .where(CrawlJob.status == "pending", CrawlJob.run_after <= utcnow())
...
async def run_job(session: Session, job: CrawlJob) -> int:
    ...
    try:
        ...
        job.run_after = utcnow() + ERROR_BACKOFF
        log.warning("job %d failed: %s", job.id, exc)
        return 0
